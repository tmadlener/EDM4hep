...
#include "edm4hep/TPCHitSIOBlock.h"
...
void TPCHitSIOBlock::read(sio::read_device& device, sio::version_type) {

  auto* dataVec = static_cast<edm4hep::TPCHitCollection*>(_col)->_getBuffer();
  ...
  podio::VectorMembersInfo * vecMemInfo = _col->vectorMembers() ;
    ...
    auto* vec0 = *reinterpret_cast<std::vector<int>**>(vecMemInfo->at(0).second);
    device.data(size);
    vec0->resize(size);
...
void TPCHitSIOBlock::write(sio::write_device& device) {
